...
import { PROJECTS, type Project, type BadgeType } from '../data/projects'
  ...
  return (
    ...
    >
      ...
      <h2 className="text-[clamp(2rem,4vw,3rem)] font-extrabold tracking-tight leading-tight mb-12 reveal opacity-0 translate-y-8 transition-all duration-700 delay-100">
        Things I've Built<br /><span className="text-gradient">& Broken</span>
      </h2>

      <div className="grid grid-cols-3 gap-5">
        {PROJECTS.map((project, i) => (
          <ProjectCard key={project.id} project={project} delay={i * 70 + 100} />
        ))}
      </div>
    </section>
  )
...
function ProjectCard({ project, delay }: { project: Project; delay: number }) {
  ...
  return (
    ...
    >
      ...
      {/* Number */}
      <div className="font-mono-custom text-[0.62rem] text-[#6b7280] tracking-[0.2em] mb-3">{project.num}</div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {project.tech.map(t => (
          <span
            key={t}
            className="font-mono-custom text-[0.58rem] text-[#00b4ff] border border-[rgba(0,180,255,0.25)] bg-[rgba(0,180,255,0.05)] px-2 py-0.5 tracking-wide"
          >
            {t}
          </span>
        ))}
